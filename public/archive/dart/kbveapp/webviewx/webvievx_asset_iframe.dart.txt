...
import 'package:admin/util/helpers.dart';

class WebViewXChart extends StatefulWidget {
  final String url;
  WebViewXChart({Key? key, required this.url}) : super(key: key);

  @override
  _WebViewXChartState createState() => _WebViewXChartState();
}
...
class _WebViewXChartState extends State<WebViewXChart> {
  ...
  List<Widget> _buildButtons() {
    return [
      ...
      createButton(
        text:
            'Change content to URL that does not allow iframes embedding\n(https://news.ycombinator.com/)',
        onTap: _setUrlBypass,
      ),
      buildSpace(direction: Axis.vertical, flex: false, amount: 20.0),
      createButton(
